...
| BUG-9 | `leases#show` PDF format | **VERIFIED broken**: `render pdf: ..., template: "leases/show.html.erb"` cannot resolve (only `show.slim` exists; lookup runs with `formats: [:pdf]`) → `ActionView::MissingTemplate`. Lease PDF *download* is dead; PDF *generation* for renewals (`HappyHouse::Leases::Generator`) works and is covered. | Pinned in `spec/requests/leases_crud_spec.rb`. The PDF-renderer replacement step should fix this, not preserve it. |
